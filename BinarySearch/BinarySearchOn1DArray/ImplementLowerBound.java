...
public class ImplementLowerBound {
     ...
     * bound of the target in this given array. The lower bound of a number is
     * defined as the smallest index in the sorted array where the element is
     ...
     * Output: 4
     * Explanation: 4 is the smallest index in arr[] where element (arr[4] = 11) is
     * greater than or equal to 11.
     * Input: arr[] = [2, 3, 7, 10, 11, 11, 25], target = 100
     * Output: 7
     * Explanation: As no element in arr[] is greater than 100, return the length of
     * array.
     * Constraints:
     * 1 ≤ arr.size() ≤ 106
     * 1 ≤ arr[i] ≤ 106
     ...
     */
     static int lowerBound(int[] arr, int target) {
        ...
        while(low<=high){
            ...
            if(arr[mid]>=target){
                ans = mid;
                high = mid-1; // find the smallest one so traverse toward the left
            }else{
                low = mid+1;
            }
        }
