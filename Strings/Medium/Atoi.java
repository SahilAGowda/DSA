    ...
    public static int myAtoi(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) >= '0' && s.charAt(i) <= '9') {
                ...
                if (result > (max - val) / 10) { // y i am dividing by 10 is to ensure that can i add the next digit if
                                                 // the curr res is exceding we would just return based on the signVal;
                    return signVal == 1 ? max : min;
                }
